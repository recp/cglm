...
CGLM_INLINE
void
glm_unprojecti_no(const vec3 pos, mat4 invMat, const vec4 vp, vec3 dest) {
  vec4 v;
...
CGLM_INLINE
void
glm_project_no(vec3 pos, mat4 m, const vec4 vp, vec3 dest) {
...
CGLM_INLINE
float
glm_project_z_no(const vec3 v, mat4 m) {
  float z, w;
